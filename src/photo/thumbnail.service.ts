...
export class ThumbnailService {
  ...
  async generateMergedWorldcupImageFontTest(imageUrls: string[]) {
    const MAX_RETRY = 2;
    for (let i = 0; i < MAX_RETRY; i++) {
      try {
        // 2. 캔버스 생성 및 이미지 병합
        const mergedImageBuffer =
          await this.generateMergedCanvasFontTest(imageUrls);
  ...
  async generateWorldcupThumbnailFontTest(imageUrls: string[]) {
    const MAX_RETRY = 2;
    for (let i = 0; i < MAX_RETRY; i++) {
      try {
        // 2. 캔버스 생성 및 이미지 병합
        const thumbnailBuffer =
          await this.generateWorldcupThumbnailCanvasFontTest(imageUrls);
  ...
  private async generateMergedCanvasFontTest(
    imageUrls: string[],
  ): Promise<Buffer | null> {
    // 1. 기본 논리 사이즈 설정 (사용자 제공 수치)
    const width = 800;
  ...
  private async generateWorldcupThumbnailCanvasFontTest(
    imageUrls: string[],
  ): Promise<Buffer | null> {
    const width = 300;
    const height = 150;
